...
export default function Lend() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (tab === "deposit") {
        // First deposit to contract - wait for confirmation
        const result = await contractDeposit(amount);
        if (result) {
          // Only record in backend after blockchain confirmation
          await api.deposit({ lender_address: account!, amount: parseFloat(amount) });
        ...
        // First withdraw from contract - wait for confirmation
        const result = await contractWithdraw(amount);
        if (result) {
          // Only record in backend after blockchain confirmation
          await api.withdraw({ lender_address: account!, amount: parseFloat(amount) });
